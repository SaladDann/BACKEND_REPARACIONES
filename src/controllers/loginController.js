...
//eliminar luego
export const probarLogin = async (req, res) => {
  try {
...
export const login = async (req, res) => {
  ...
  if (!username || !password) {
    return res.status(400).json(response_bad_request("Username y password son requeridos"));
  }
  ...
  try {
    ...
    if (!usuario || !usuario.Usuario_activo) {
      return res.status(400).json(response_bad_request('Usuario no encontrado o inactivo'));
    }

    const passwordValida = await bcrypt.compare(password, usuario.Usuario_Password);

    if (!passwordValida) {
      return res.status(400).json(response_bad_request('Contraseña incorrecta'));
    }
